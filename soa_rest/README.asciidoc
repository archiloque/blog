...
Aujourd'hui, les parties techniques des services ne sont plus un problème : on dispose d'outils matures pour les développer et les exploiter facilement.

Lorsqu'on décide de généraliser l'usage des services au niveau d'un système d'information, les enjeux sont de deux ordres :

- avoir des services qui répondent le mieux aux besoins
- garder la maîtrise de son SI dans la durée

Même si les outils ont changé, ces besoins des entreprises en terme d'architectures de services sont restés identiques,
...
Les urbanistes planificateurs des années 2000 ont laissé la place à une approché décentralisée :
une stratégie et des grands principes globaux puis une autonomie des projets.

== L'urbanisation c'est pour qui ?

L'urbanisation vous concerne dès que vous commencez à mettre en place des services. 
...
Pour atteindre ces objectifs, voici 3 patterns qui ont fait leurs preuves :
...
Le mieux est de définir une règle de répartition simple, en considérant que sur le long terme les choses vont s'équilibrer.
...
Pour le moment il faut donc customiser les outils existants, ou utiliser les outils JSON s'appuyant sur des XSD.
...
l'objectif étant qu'ils soient plus pérennes que les services standards, car les migrations sont plus compliquées.
