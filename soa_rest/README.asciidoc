...
Les deux situations où les pratiques vitales sont les plus utiles :
...
Il s'agit d'un modèle plus contractuel, où chaque application est responsable de ses données et définit seule les services qu'elle expose.
Les contacts entre applications reposent alors sur un modèle client-fournisseur formalisé plutôt que des échanges collaboratifs.
...
_Ici il a été décidé le cœur métier est le mieux placé pour définit le modèle canonique des produit et des clients, car c'est lui qui embarque le plus de logique, le CRM et la BI utilisent ce modèle pour leurs échanges et peuvent l'enrichir pour leurs propres services._
...
- La réutilisation des modèles va permettre d'offrir une API consistante à vos différents services, même si en interne ils sont issus d'applications totalement différentes.
