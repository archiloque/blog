...
Il ne s'agit pas d'un modèle link:https://fr.wikipedia.org/wiki/Modèle-vue-contrôleur[MVC] : les définitions des écrans sont en fait imbriquées avec le code des programmes contenant les traitements métiers.
...
Il ne s'agit bien entendu pas d'API REST, mais généralement de messages MQ permettant d'exposer les programmes Cobol ou directement les écrans Mainframe.
...
Dans ce cas les données dans la base du site web qui n'ont pas été déversées dans la base du Mainframe ne sont pas visibles depuis le backoffice.
...
C'est le cas de multi-canalité qu'on retrouve le plus souvent.
...
. les canaux doivent implémenter ces mêmes règles au niveau de chaque écran ou de chaque service exposé avec la granularité la plus finle possible pour être en mesure de remonter des erreurs au plus près.
...
Il existe de multiples approches, celles qui sont mentionnées ici ont été choises car elles mettent en lumières les contraintes qui s'appliquent.
...
Les BPM sont des outils permettant de définir des worklow métiers sous forme _low-code_, c'est à dire via de la configuration et/ou des designers graphiques.
...
Deux point d'attention pour cette approche :
...
Une des difficultés de cette stratégie est de bien choisir l'ordre dans lequel remonter les fonctionalités pour bénéficer au plus vite des premiers avantages tout en limitant les risques.
