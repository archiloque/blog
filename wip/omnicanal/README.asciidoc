...
Le système Mainframe historique est vital pour l'entreprise, et la maîtrise qu'on en a n'est pas toujours satisfaisante : ce patrimoine commence à dater, sans test automatisé (ce qui est normal vu l'époque) et avec une documentation souvent lacunaire.
...
Il ne s'agit bien entendu pas d'API REST, mais généralement de messages link:https://fr.wikipedia.org/wiki/IBM_MQ[MQ] permettant d'exposer les programmes Cobol.
...
Ces partenaires peuvent par exemple vendre des prêts de la lorsque vous achetez un de leur produits.
