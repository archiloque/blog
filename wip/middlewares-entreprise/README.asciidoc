...
Même si vous n'avez pas besoin d'ajouter ou de supprimer des instances à tout bout de champ, une augmentation de trafic peut nécessiter d'ajouter une instance d'ici quelques mois et il est important d'en connaître les impacts.
...
selon les outils, changer le nombre d'instance peut être totalement transparent, peut diminuer les performances pendant le rééquilibrage de données, ou peut même nécessiter de redémarrer tous les serveurs pour propager une configuration.
