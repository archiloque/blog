...
Soit parce qu'on a besoin d'implémenter quelque chose alors que vos connaissances ne le permettent pas, ou pire, qu'un bug dans une des abstractions a fait planter vos serveur de productions.
...
La manière historique de le faire est d'acheter du support pour les logiciels, c'est à dire de payer pour avoir la capacité d’appeler quelqu'un quand il y a un problème : l'empilement est toujours là mais vous n'êtes pas seul·e face à lui.
