...
Après avoir link:../moteur-de-taches-en-ruby-2-boucle-de-traitement[construit la base de la boucle de traitement] et je vais maintenant m'intéresser à la persistance.
endif::[]
...
Il est possible d'utiliser deux systèmes différents pour les deux besoins, par exemple pour les tâches pas encore lancées il n'est pas rare d'utiliser un bus de messages, car en plus du stockage il peut servir de médiateur entre  un système qui créé des tâches et le {mdt} en fournissant une API facile à utiliser.
...
J'ai choisi d'utiliser PostgreSQL car le fonctionnement des transactions des bases de données SQL sera utile pour éviter la perte de données et gérer la parallélisation, et que PostgreSQL fournit un système de notification permettant de répondre au troisième point.

Pour l'accès aux données je vais utiliser un ORM.
Plutôt qu'ActiveRecord qui amène avec lui tous l'écosystème Rails, je vais utiliser link:http://sequel.jeremyevans.net[Sequel].
Sequel n'est pas aussi complet qu'ActiveRecord mais il embarque beaucoup moins de code tout en fournissant ce dont j'ai besoin en utilisant des patterns qui sont proches, tout en me permettant de mieux maîtriser le SQL généré.
...
Après exécution, les tâches seront supprimées de la base.

Pour le moment je vais continuer d'utiliser les mêmes tâches qui se contentent de mettre le worker en attente, ce qui signifie que je n'ai pas à gérer de paramètres.
...
Lors de la configuration de l'accès à la base de donnée, j'indiquerai d'utiliser au maximum une connection par worker plus une pour le moteur.
Cela permet que chaque worker ait une connection à lui plutôt que de risquer que les worker ne s'attendent pour pouvoir faire des requêtes.

Pour le moment je vais tout conserver dans le même fichier{nbsp}:
...
module TaskEngine
  ...
  # Use at most one connection per worker plus one for the engine
...
Une chose importante dans le design des tâches{nbsp}: il est souhaitable&#8201;—{nbsp}tant que cela est possible{nbsp}—&#8201;qu'une même tâche puisse être exécutée plusieurs fois de suite sans que cela soit génant.
...
Cela vient du fait qu'il est très difficile de faire en sorte que le {mdt} ne perde pas de tâche et en même temps n'exécute pas la même tâche deux fois, même en cas d'erreur ou de problème réseau.
...
Le {mdt} se contentera de faire en sorte que la même tâche ne soit pas exécutée deux fois en même temps.

Plutôt que d'essayer de faire en sorte d'avoir un système qui garantit du "`exactly once`", on préfère que le {mdt} se concentre sur le fait de ne pas perdre de tâche, et si une même tâche est exécutée une deuxième fois c'est la tâche qui est chargée que cela n'ait pas d'effet gênant.
...
L'autre avantage de cette approche est qu'en cas de gros problème, il devient possible de réinjecter dans le {mdt} toutes les tâches passées et de les laisser les traiter, sans avoir à se préoccuper de celles qui avaient déjà été exécutées.
...
Juste après avoir mis à jour le statut de T1, on la déverrouillera dès que possible pour débloquer les autres threads.
...
Le relâchement du verrou se fait à la fin de la transaction en cours, qui doit donc être faite dès que possible pour limiter la période pendant laquelle les autres workers risquent d'être en attente.

L'acquisition de la tâche (sélection et mise à jour du statut) doivent de toutes façon se faire dans une transaction séparée de celle de l'execution de la tâche{nbsp}:

- pour l'acquisition soit immédiatement visible dans la base{nbsp};
- pour qu'en cas d'erreur dans la tâche qui déclencherait un `rollback`, l'acquisition ne soit pas automatiquement annulée, ce qui rendrait plus difficile la reprise.
...
Lorsque l'execution se termine, pas besoin de poser de verrou car il n'y a pas de risque qu'un autre worker veuille terminer la même tâche.
Par contre il est toujours nécessaire d'isoler le code dans une transaction à part, notamment pour qu'une erreur dans le {mdt} puisse avoir une conséquence sur la tâche qui vient d'être exécutée.
...
ifeval::["{doctype}" == "book"]
Dans la partie suivante je vais m'intéresser à l'arrêt et et au redémarrage du moteur.
endif::[]
ifeval::["{doctype}" != "book"]
link:../moteur-de-taches-en-ruby-4-extinction/[Dans l'article suivant] je vais m'intéresser à l'arrêt et et au redémarrage.
endif::[]