...
Cela permet que chaque worker ait une connexion à lui plutôt que de risquer que les worker ne s'attendent pour pouvoir faire des requêtes.
...
module TaskEngine
  ...
  # Use at most one connexion per worker plus one for the engine
  DB = ::Sequel.connect(DATABASE_URL, max_connexions: WORKERS_NUMBER + 1, logger: LOGGER)
...
Une chose importante dans le design des tâches{nbsp}: il est souhaitable&#8201;—{nbsp}tant que cela est possible{nbsp}—&#8201;qu'une même tâche puisse être exécutée plusieurs fois de suite sans que cela soit gênant.
...
Plutôt que d'essayer de faire en sorte d'avoir un système qui garantit du "`__exactly once__`", on préfère que le {mdt} se concentre sur le fait de ne pas perdre de tâche, et si une même tâche est exécutée une deuxième fois c'est la tâche qui est chargée que cela n'ait pas d'effet gênant.
...
ifeval::["{doctype}" != "book"]
link:../moteur-de-taches-en-ruby-4-extinction/[Dans l'article suivant]
endif::[]
je vais m'intéresser à l'arrêt et et au redémarrage du moteur.