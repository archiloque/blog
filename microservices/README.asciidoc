...
Si l'exemple-type est d'avoir une unité de déploiement par projet, la régle est plutôt de n'avoir _au plus_ qu'une équipe par unité de déploiement.
Si domaine métier bien découpé est implémenté en plusieurs applications séparées, elles seront portées par la même équipe.
