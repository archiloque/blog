...
Tout cela créé des pertubations dans les plannings et rend les durées d'execution de tâches et les dates de fin de projets difficiles à prévoir, même alors que le modèle est pensé dans cet objectif.

Même dans les cas favorables, créer un logiciel n'est donc pas une chaîne d'assemblage, méfiez-vous donc des enthousiastes du lean.
