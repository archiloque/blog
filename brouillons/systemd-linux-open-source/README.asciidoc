...
{s}&#8201;—{nbsp}développé d'un seul tenant{nbsp}—&#8201;va donc à l'opposé de ce principe.
...
Beaucoup de supposition fait dans la manière dont les composants devraient se comporter par défaut, du coup même si on peut faire différemment, le chemin de moindre résistance cela impulse une manière de faire.
...
Surtout il développe des logiciels Linux qui&#8201;—{nbsp}comme {s}{nbsp}—&#8201;vont à l'encontre des principes Unix et qui répondent à des problèmes connus, et il a de l'énergie et la capacité à s'investir jusqu'à convaincre les personnes qui sont en position de choisir de les utiliser (par ex celles qui font du packaging), tout en choisissant de ne pas traiter tous les besoins

Il s'agit d'une approche qui a fait ses preuves, mais qui fâche des gens car elle n'est pas consensuelle.

Du coup quand il a commencé à proposé {s}, en plus des arguments décris plus haut le fait que ça soit lui a cristalisé des mécontentements, surtout qu'à mon avis, connaissant sa capacité à aller jusqu'au bout et à convaincre, une partie des personnes a du réaliser que la partie était probablement perdue d'avance.

=== Les alternatives

D'autres systèmes alternatifs qui existaient et qui répondaient à certaines critiques de {syi}, par exemple link:https://wiki.gentoo.org/wiki/OpenRC[https://wiki.gentoo.org/wiki/OpenRC]. Malheuresement ils ne supportaient pas les choses les plus avancées qui sont dans systemd, ce qui fait que même s'ils étaient plus proches de {syi} et qu'ils promettaient donc une migration moins compliquées, les personnes concernées (dev et admin sys) ont estimés que le jeu n'en vallait pas la chandelle.
