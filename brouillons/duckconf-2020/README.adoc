...
Je vais commencer par parler en général, puis je résumerai les éléments que j'ai noté sur les différents talks.
...
=== Les talks{nbsp}: peu mieux faire
...
Dans son raisonnement, Thomas essaie de montrer les effets induits par l'augmentation du nombres de personnes sur le développement, mais pour cela il mélange plusieurs choses, d'un part ce qui augmente avec le nombre de personnes mais en pourcentage constant et ce qui augmente en pourcentage, et d'autre part les choses qui arrivent dans tous les cas avec celles qui risquent d'arriver.
...
Le discours m'a fait sourir car quand les solutions d'API management sont sorti, une partie des architectes expérimentés les comparaient aux solutions link:https://fr.wikipedia.org/wiki/Universal_Description_Discovery_and_Integration[UDDI] qui fournissaient des portails développeur·euse·s pour du SOAP et qui avaient plutôt été un désastre.
...
Pascal Martin raconte la migration de 6play, le service de replay de M6 et d'autres télévision depuis leurs serveurs physiques vers le cloud.
...
Les deux speakers montrent que dans un delivery automatisé, chaque étape peut s'appuyer sur différents outils permettant de vérifier tel ou tel aspects de la sécurité de l'application.
...
J'aurais aimé que les auteurs donnent un peu plus leur avis sur la pertinence des nombreux outils, et sur la manière de prioriser les mise en œuvre, car du coup le talk fait un peu un effet catalogue.

== Emmanuel Toulemonde & Mehdi Houacine{nbsp}:mise en prod de la data science{nbsp}: le jour d'après
