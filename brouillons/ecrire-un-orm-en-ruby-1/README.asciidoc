...
Avant de plonger dans le code, je vais ici introduire les sujets et expliquer certains des choix de design.

== Les ORMs

Un ORM est une couche qui fait le lien entre une base de données et la représentation des données dans un programme.
Elle permet d'automatiser ou de simplifier certains actions, notamment la génération de requêtes, et enrobe les appels à la base dans une API dans le style du langage de programmation utilisé.

Elle peut aussi fournir une compatibilité entre plusieurs systèmes de base de données, et un système de cache.

Les ORMs sont probablement les outils les plus complexes auxquels on a affaire quand on commence à développer une application web.
...
Impression souvent renforcé quand on lit le code de ces outils.
...
Voila pour l'introduction, dans l'article suivant je vais m'occuper à la structure du projet.