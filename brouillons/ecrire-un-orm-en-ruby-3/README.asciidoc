...
En effet même si tous les ORMs n'utilisent pas les métadonnées beaucoup le font et il est utile de comprendre que cela n'est pas si compliqué.
...
Avec SQLite, l'accès aux métadonnées se fait via link:https://www.sqlite.org/pragma.html[les commandes PRAGMA].
...
Dans le template, `columns_definitions` contient alors la liste des `ColumnDefinition` prêtes à l'emploi.
...
Je vais tout de même nommer ma méthode `truncate` pour qu'elle corresponde à la commande SQL standard, même si l'implémentation n'utilise pas exactement cette requête dans SQLite.
...
Je ne vais pas les détailler ici mais pour les suppressions d'enregistrements il faut générer une requête `DELETE FROM table_name WHERE ID = ?` et lui passer l'id de l'enregistrement et pour les mises à jour s'inspirer de la requête d'insertion pour écrire une requête du type `UPDATE table_name SET column_name_1 = ?, column_name_2 = ? WHERE id = ?`.
