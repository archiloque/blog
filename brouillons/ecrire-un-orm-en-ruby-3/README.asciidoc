...
En effet, même si tous les ORMs n'utilisent pas les métadonnées, beaucoup le font il est donc important de comprendre comment comment cela fonctionne et de réaliser que ça n'est pas si compliqué{nbsp}…
...
Je ne vais pas les détailler ici mais pour les suppressions de modèles il faut générer une requête `DELETE FROM table_name WHERE ID = ?` et lui passer l'id de du modèle et pour les mises à jour s'inspirer de la requête d'insertion pour écrire une requête du type `UPDATE table_name SET column_name_1 = ?, column_name_2 = ? WHERE id = ?`.
