...
Fait un schéma d'architecture globale puis l'ont découpé.

Ont commencé par des transfert de données manuels et sans CI.

Lutte qui semble dure face à l'architecture.

=== Tour d'horizon des algorithmes de consensus en 2021 / Ameyric Benthencourt & Franck Hillard

=== CQRS à notre secours / Florent Jaby

Un déroulé d'une mise en place d'un process CQRS.

=== Table ronde{nbsp}: les coûts dans le Cloud

Couteux : se réapproprier les plateformes et les aspects humains

Gros impacts des applis pas conçues hors du cloud

Le cloud simplifie certaines choses mais la multiplicité des choix 

Gestion de la finance par l'équipe en charge de la plateforme

J'étais un peu étonné de l'idée que les fournisseurs de Cloud fait payer en fonction du coût et pas en fonction de la valeur du client
