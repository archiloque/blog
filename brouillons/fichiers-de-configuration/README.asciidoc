...
Cela ressemble grosso-modo à une phase de compilation.
...
Même si votre format de configuration fournit des fonctionnalités avancées de factorisation, les personnes vont vouloir générer ces fichiers à partir d'un autre outil.
...
La deuxième règle étant bien plus importante que la première cet implémenter une gestion des échappements demande du code assez minutieux et bien testé, alors qu'en 
