...
It's natural to represent a position in such an array as a single integer. Compared to a more traditional representation as a pair of coordinates, this saves having to create an object each time we want to return a position from a function or pass one as an argument, specially in Java were there are no struct and only full-fledged classes.
