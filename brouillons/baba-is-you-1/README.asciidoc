...
To convert a line/column coordinate to a position and back you need to use this code:

[source,java]
----
int line = position / levelWidth;
int column = position % levelWidth;

int position = (line * levelWidth) + column;
----
...
Now the code read a level from disk and I'll start implementing the solving logic in the next article.