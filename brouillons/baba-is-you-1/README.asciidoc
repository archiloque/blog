...
:docdate: 2019-05-09
:article_lang: en
:ignore_files: content.txt, graph1.asciidoc, graph1.mmd, graph1.png, levelreader1.java, levelreader2.java, levelreadertest.java, mvn.txt, tiles.java, tiles.txt
...
Levels use a square grid, and there is at most one item per cell in the starting position.
...
Note: I'd like to rely on emojis to use visual representations of the objects, but the way macOS deal with emoji don't make them monospace friendly, so it breaks the visual aspect of the level files and editing them becomes a pain.
...
You lose some type checking this way and makes code a bit less readable but it's an acceptable downside for me.
