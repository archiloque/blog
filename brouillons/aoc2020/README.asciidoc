...
As implied by its name, they are supposed to be done during the advent.
A two-parts problem is published each day, and you can even compete on solving them as fast as possible.
This process help some people to keep their motivation all along the 25 days.

I hate the peer pressure and the idea of self-imposed mandatory homework to do everyday, so as each year I do them later so I can choose my pace to work on them.
...
This year I've decided to use the advent of code to practice link:https://nim-lang.org[Nim].

Ruby is still my language of choice, but I needed another tool for specific _personal_ projects I want to try.

So since a few years I was looking for a programming language with these requirements:
...
- Garbage-collected
- Mature enough to not require too much busy work to keep my code up to date
- Not managed by a large corporation with terrible practices
- A welcoming community free of obnoxious people

Nim is an interesting tool, to me it looks like +++<del>+++Python vored C+++</del>+++ a mix of Python and C, where you can have Python expressivity with C-level performance.
