...
image::images_list_no_extension[]
...
Les choses se compliquent encore car les liens entre pages forment souvent des cycles, par exemple si la page A est la page de sommaire du site, il y a des chances que toutes les pages vers lesquelles A pointent vont à son tour pointer vers elle.
Donc pour traiter A il y a besoin d'avoir traité de B mais pour traiter B il y a besoin d'avoir traité de A.

La solution est une approche en deux passes :
