...
def download_path(file_name)
  target_file_path = full_file_name(file_name)
...
def scrape_resource(html_page_url, resource_url)
  ...
  absolute_url = html_page_url.join(resource_url)
  puts "Vérifie la ressource [#{absolute_url}]"

  file_name = calculate_file_name(absolute_url)
...
until waiting_pages_urls.empty?
  ...
  if current_page_file_path
    ...
    doc.css('a').each do |a|
      if a.key?('href')
        href = a['href']
        absolute_href = current_page_url.join(href)
