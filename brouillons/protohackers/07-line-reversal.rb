...
class Session
  ...
  def find_data
    ...
    @last_message_start.upto(@content.length - 1) do |index|
      ...
      if current_char.nil?
        ...
        content.concat(initial_data.map do |c|
          case c
          when '/'
            '\\/'
          when '\\'
  ...
  def send_data_packets
    packet_start_index = @last_message_start
    data = self.find_data
    if data.empty?
      log("No new data")
      return
    end
    log("#{data.length} bytes to send")
    data.each_slice(950) do |data_slice|
      if @waiting_acks.key?(packet_start_index)
        log("Already sent packet #{packet_start_index}")
      else
  ...
  def received_ack(position)
    if position > @last_message_start
      log("Received ack: misbehaving so closing the session #{position} #{@last_message_start}")
      SOCKET.send("/close/#{@session_id}/", 0, @ip_address, @port)
      SESSIONS.delete(@session_id)
    elsif position == @last_message_start
      @max_ack_received = position
      log("Received ack: final #{position} #{@last_message_start}")
    elsif @waiting_acks.key?(position)
      if position > @max_ack_received
        @max_ack_received = position
      end
      log("Received ack: received #{position} #{@last_message_start}")
    else
      log("Received ack: resending packets #{position}")
  ...
  def heartbeat(current_time)
    @waiting_acks.keys.each do |position|
      ...
      unless ack.nil?
        if position < @max_ack_received
          log("Heartbeat nullify ack #{position}")
          @waiting_acks[position] = nil
        else
          delta = (current_time - ack.timestamp)
          log("Heartbeat #{position} delta #{delta}")
          if delta == 0
          elsif delta >= 60
