...
J'entends régulièrement ces questions, pour lesquels on aimerait des réponses toutes faites.
...
C'est parce que mon approche consiste à répartir les choses d'une manière qui réponde au mieux aux besoin de l'équipe et de ses membres.

Une question comme « est-il légitime qu'un architecte SI passe du temps à coder ? » qui est une question de statut n'a pas grand chose à voir là dedans :
