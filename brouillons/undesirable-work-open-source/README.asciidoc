...
The first example, which I already wrote about link:https://archiloque.net/blog/systemd-linux-open-source/[here] in French, is systemd, which is a family of components that you can use to manage lots of the grunt work required to run Linux on a machine, like dealing with starting and stopping other components.

Systemd's approach is disliked by many Linux developers.
On the other hand, Systemd provides a standardized way to do things that before it tended to be different between Linux distributions.
...
Some of them were ideological and others involved real technical problems or limitations of systemd.
