...
Just as the `Level`, it starts out as a small class with few attributes (the current status of the game and a reference to the `Level`) and it will become larger as I add features. For instance, this is where I'll handle Baba's dynamically changing rules.
...
Rather than depend on an existing FIFO implementation, I'll just roll my own. It's short and it does exactly what I need and nothing more.
////
Notes de Fred-le-sceptique :
- En tant que lecteur, je ne vois pas vraiment ce que m’apporte le code de la Queue, ça me semble plutôt annexe par rapport au fait d’apprendre à écrire un solveur de jeux
- Et j’avoue que je ne suis pas convaincu de l’intérêt d’écrire une implem maison sachant qu’il y en a déjà dans le JDK
- Les perfs des linked list sont généralement dégueu à cause de la mauvaise localité de cache. Il faudrait profiler, mais je ne serais pas surpris que le temps passé à empiler / dépiler des éléments ait une influence significative sur la vitesse du solveur
- L’un dans l’autre, je pense que je commencerais par utiliser juste une ArrayDeque et puis baste :-)
////
