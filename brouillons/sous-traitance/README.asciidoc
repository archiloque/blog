...
:sous: l'entreprise sous-traitante
:Sous: L'entreprise sous-traitante
:ordre: l'organisation donneuse d'ordre
:Ordre: L'organisation donneuse d'ordre
...
Je vais parler ici du cas où {sous} est payée, de manière directe ou indirecte, en fonction de la quantité du temps qu'il passe.
Par exemple lorsque {sous} réalise des lots dont la taille et donc le prix sont négociés en fonction d'une estimation du temps de travaille qu'ils demandent.
J'exclus donc le cas du projet entièrement au forfait, et le cas de la régie où les employé·e·s de {sous} travaillent directement pour {ordre}, comme s'il·elle·s étaient directement ses employé·e·s.

== {Sous} n'est pas ton amie

Souvent, quand une partie conséquente d'un projet (voir l'ensemble du développement) est déléguée à une entreprise tierce, on appelle {sous} "`partenaire`".
...
Mais pour chacune unité de travail sous-traitée, il y a une opposition frontale sur le prix{nbsp}: {ordre} veut payer le moins cher possible, et {sous} veut être payée le plus cher possible.

Après tout est affaire de patience, de rapport de force, et de savoir bien rédiger les clauses du contrat.

La négociation consiste souvent pour {sous} à essayer de déterminer le prix maximum que {ordre} est prête à payer.
...
Je ne dis pas que {sous} va nécessairement se conduire d'une manière extrême&#8201;—{nbsp}par exemple en surévaluant les tâches qui lui sont confiées{nbsp}—&#8201;mais que son intérêt objectif peut l'amener à le faire.
...
J'ai ainsi croisé plusieurs projet où les tests automatisés étaient absents ou presque inexistants et où {sous} se débrouillait pour ne pas en écrire malgré les demandes régulières.
{Ordre} payait à chaque fois {sous} pour une campagne de test manuels à chaque nouvelle version, et en général finançait également une partie des corrections, qu'on faisait semblant de considérer comme des évolutions (car les vrais bugs sont en principe corrigés sans coût supplémentaire car ils sont considérés comme des erreurs de {sous}).

En dehors du coût, cela signifie que&#8201;—{nbsp}dans certains cas{nbsp}—&#8201;l'intérêt financier de {sous} peut mener à des logiciels moins fiables et moins faciles à faire évoluer.

La non-réutilisation peut même être tentante pour {ordre}, car elle peut simplifier les négociations sur le prix des évolutions.
...
Pour chaque nouvel écran, la question va donc se poser pour savoir si {ordre} accepte de payer le plein tarif, ou si elle va investir du temps et de l'énergie pour tenter de négocier une réduction en justifiant de la possibilité de réutiliser un écran existant.

== {Sous} en sait souvent plus que toi



== {Sous} rend les choses plus compliquées