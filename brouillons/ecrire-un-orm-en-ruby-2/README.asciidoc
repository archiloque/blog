...
Les noms de la table et des colonnes sont maintenant à disposition.
...
On a ici un exemple où l'ORM doit assurer la compatibilité entre les bases.
Si ce cas est assez simple, il permet de comprendre la manière dont les choses pourraient être mise en œuvres{nbsp}:une méthode de base qui utiliserait la commande `truncate`, et une classe spécifique à SQLite qui utiliserait la requête `delete`.
...
Pour l'article suivant, je vais enrichir les méthodes de récupération pour pouvori ajouter des filtres et trier les données.