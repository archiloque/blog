...
Pour une nouvelle fonctionalité du projet A, il a besoin d'une fonctionalité d'un projet B qui n'existe pas encore : un service manquant, une donnée incomplète …

Pour cela il faut faire une demande à B, que B accepte la demande, que B la réalise et mette la fonctionalité à ma disposition.
...
Je pense qu'il n'y a pas de recettes magiques, mais des pratiques peuvent aider.

== Côté demandeur

Si vous est du côté des projets demandeurs, ce que vous pouvez faire

* Connaître suffisament le système pour prédire suffisament bien les besoins qui vont solliciter une dépendance : cela évite les mauvaises suprises
...
* Faire en sorte de facilité le développement de ces nouvelles demandes, par exemple par la capacité à founir des environnement de tests pendant les phases de mise au point
* Appuyant les projets quand ils ont besoin d'escalader l'échelle hiérarchique : quand le projet A a besoin du projet B, la meilleure personne à convaincre est souvent celle qui est au croisement des deux branches de l'organigramme
* Tracer ce type de démandes pour avoir de la visibilité sur le problème : est ce qu'il arrive souvent, à quels endroits ?
* Essayer de piloter la décentralisation des données pour limiter le nombre de guichets, un peu de centralisation sur les données "maîtres" permet de faciliter les choses
