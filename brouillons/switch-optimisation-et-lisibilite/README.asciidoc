...
Pourquoi ne pas plutôt rendre public toute cette tuyauterie interne pour permettre au compilateurs de les exploiter au mieux{nbsp}?
Les compilateurs disposent de tout le code à exécuter, et devraient pouvoir faire de meilleurs optimisations, et peuvent investir plus de temps pour leur compilation.
L'autre avantage est que cela pourrait permettre de simplifier les processeurs et donc de réduire leur coût et leur consommation électrique.
...
Pour pouvoir tirer partie d'un Itanium, un compilateur doit donc déduire des informations qui ne sont pas dans le code source d'origine, comme c'est le cas pour les moteurs JavaScript.
...
Cela correspond donc à un retour vers des optimisations explicites.

Même avec un processeurs non-Itanium, le code généré peut s'exécuter plus efficacement car le compilateur du processeur peut produire du code plus efficace depuis du code machine issu du Rust que depuis du code machine provenant de C.
